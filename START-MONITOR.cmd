...
REM Имя модуля.
SET PROGRAM_NAME=nsd

REM Каталог запуска скрипта.
SET RUN_DIR=%~dp0

REM ==================================================
REM Проверим рабочее время.
CALL "%RUN_DIR%CHECK-WORKING-HOURS.cmd" || (
CALL "%RUN_DIR%LOG.cmd" "%PROGRAM_NAME%" "INFO" "NOW IS NOT WORKING TIME"
EXIT /B 1
)

REM ==================================================
REM Проверим рабочий день.
CALL "%RUN_DIR%CHECK-HOLIDAY.cmd" || (
CALL "%RUN_DIR%LOG.cmd" "%PROGRAM_NAME%" "INFO" "TODAY IS NOT WORKING DAY"
EXIT /B 1
)
